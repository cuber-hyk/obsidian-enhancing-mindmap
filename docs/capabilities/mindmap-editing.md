...
- 节点剪贴板优先识别现有 `copyNode` JSON 格式；未匹配内部格式时，标题、无序列表、有序列表和普通文本会转换为当前活动节点下的节点森林。多个 Markdown 顶层条目按原顺序成为多个直接子节点，嵌套条目保持层级；纯普通文本的每个非空、非缩进行成为一个同级子节点，空行忽略。带缩进的纯文本不推断层级，结构化 Markdown 与普通文本混合时整体拒绝。
...
- Markdown 层级粘贴需在测试 Vault 中验证标题、无序列表、有序列表、单行及多行普通文本、空行忽略、多个顶层节点、深层嵌套、新 ID、最终文本宽度与底部连线对齐、折叠目标、连续粘贴、一次撤销/重做、缩进纯文本拒绝、结构与普通文本混合拒绝、异步期间切换节点或 leaf，以及内部 `copyNode` JSON 的优先级回归。
