...
- `MindMapShortcutCatalog` 是脑图局部快捷键的唯一配置源，集中维护 42 个动作的默认绑定、分类、高频标记和适用上下文；旧 `nodeKeyboardShortcuts` 中两个同级新增绑定在首次加载后迁入新配置并停止双轨维护。标题栏右侧快捷键检查器只显示 13 个高频动作，全集使用独立 Modal 搜索、分类、录制、清除、筛选已修改项，以及按单项、分类或全部恢复默认；设置页仅提供全集入口和全部恢复。两处保存后立即同步所有已打开脑图。
- `MindMapShortcutRouter` 只在当前活动 `MindMapView`、事件目标属于脑图容器且动作上下文匹配时分发一个局部动作；节点选中、节点编辑、图片聚焦、多选和空白画布分别判定。输入框、按钮、链接、插件 Modal、非节点编辑器及非活动 leaf 保留宿主行为；互斥上下文可以复用同一按键，存在上下文交集的重复绑定会被拒绝。
...
- 编辑态已选中的图片可拖到节点正文上方或下方，拖动时显示对应边界的插入指示并阻止节点整体拖动；聚焦图片后可使用默认的 `Alt+↑/↓/←/→` 将图片放到文字对应方向，四项均可在脑图局部快捷键全集中重新绑定。
...
- `Copy Node`、`Cut Node`、`Paste Node`、`Undo` 和 `Redo` 等插件命令继续保留在命令面板，用户在 Obsidian 中主动设置的全局绑定不会被插件改写；插件自身不再为任何命令声明全局默认热键。脑图局部配置默认使用 `Ctrl`/`Cmd+C/X/V/Z`，Windows/Linux 重做为 `Ctrl+Y`、macOS 重做为 `Cmd+Shift+Z`；History 只在活动脑图的画布或节点选中上下文执行，节点文字编辑和普通 Markdown 编辑器保留编辑器原生撤销。
...
- 脑图局部快捷键目录与迁移：`src/mindmap/interaction/MindMapShortcutCatalog.ts`
- 脑图局部快捷键路由：`src/mindmap/interaction/MindMapShortcutRouter.ts`
- 常用面板与全集管理器：`src/mindmap/interaction/MindMapShortcutInspector.ts`、`src/mindmap/interaction/MindMapShortcutManagerModal.ts`
...
- 节点键盘状态机、链接编辑/删除以及图片预览/缩放/删除需验证根节点、普通节点、编辑态、多链接、多图片、关闭后焦点恢复、撤销/重做和 Markdown 往返；快捷键回归还需验证右侧 13 项高频投影、全集搜索与筛选、录制/清除/分层恢复、上下文冲突拒绝、互斥上下文复用、旧设置迁移、即时多视图同步、重开保持、深浅主题，以及 Markdown 编辑器、节点正文和 Modal 的原生键盘行为。
...
- 键盘焦点与 History 回归需覆盖空白画布撤销/重做、Windows/Linux `Ctrl+Y`、macOS `Cmd+Shift+Z`、四向与 `Home` 导航、叶节点及已展开节点不新增 History，以及导航器、快捷键检查器、全集 Modal、设置页和其他交互控件保留宿主键盘行为；还需确认 Obsidian 快捷键页不存在插件声明的默认绑定，用户主动设置的宿主全局绑定保持不变。
