...
- 脑图标题栏的键盘操作打开或关闭当前视图内的右侧快捷键检查器；快捷键检查器与样式检查器互斥，展示节点键盘操作，并允许录制、保存和恢复“新增下方同级节点”与“新增上方同级节点”的全局绑定。绑定写入插件设置、立即同步至已打开脑图；固定节点操作、重复绑定和纯修饰键会被拒绝。
...
- 节点键盘状态机、链接编辑/删除以及图片预览/缩放/删除需验证根节点、普通节点、编辑态、多链接、多图片、关闭后焦点恢复、撤销/重做和 Markdown 往返；快捷键检查器还需验证默认上/下方新增、录制、冲突拒绝、恢复默认、即时多视图同步、重开保持与深浅主题。
