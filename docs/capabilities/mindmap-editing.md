...
- 节点复制内容可连续粘贴，剪切只有在系统剪贴板写入成功后才删除源节点。异步剪贴板完成后还必须保持同一活动 leaf 和同一活动节点，否则不在后台删除或粘贴。
- `Copy Node`、`Cut Node`、`Paste Node`、`Undo` 和 `Redo` 命令继续保留并支持用户自定义热键；`Undo` 默认使用 `Ctrl`/`Cmd+Z`，`Redo` 默认使用 Windows/Linux 的 `Ctrl+Y` 与 macOS 的 `Cmd+Shift+Z`。History 命令只在活动 leaf 为当前脑图、没有节点编辑、焦点位于脑图容器且不在交互控件内时执行；静止空白单击会清空选择并聚焦画布，因此无选中节点时仍可撤销或重做。不再提供 `Alt+Shift+C/X/V/Z` 默认绑定。
