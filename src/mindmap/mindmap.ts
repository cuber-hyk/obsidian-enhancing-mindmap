...
export default class MindMap {
    ...
    scheduleInitialLayoutIfReady() {
        ...
        this.initialLayoutFrame = requestAnimationFrame(() => {
            this.initialLayoutFrame = null;
            if (this.isBuildingInitialTree || this.pendingInitialNodes.size > 0) return;

            this.traverseBF((node: INode) => {
                node.refreshBox();
    ...
    scheduleRenderedLayout() {
        if (!this.isInitialLayoutReady) return;
        if (this.renderLayoutFrame !== null) return;
    ...
    layout() {
        if (!this.mmLayout) {
            this.mmLayout = new Layout(
                this.root,
                this.setting.layoutDirect||'mind map',
                this.colors,
    ...
    performLayoutRefresh() {
        ...
        const visibleNodes = this.root?.getShowNodeList?.() || [];
        const boundsResult = this.canvasBoundsController?.ensureVisibleNodes(visibleNodes);
        if (boundsResult?.shifted) {
            this.mmLayout?.layout(
                this.root,
                this.setting.layoutDirect || this.mmLayout.direct || 'mind map',
            );
            this.canvasBoundsController?.ensureVisibleNodes(
                this.root?.getShowNodeList?.() || [],
                false,
            );
        }
        if (isInitialLayout) {
    ...
    restoreRuntimeCanvasBounds() {
        this.canvasBoundsController?.applyCurrentDimensions();
    }
